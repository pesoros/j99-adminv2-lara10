    ...
    <div class="card-body row">
      <div class="col-sm-12">
        <div class="form-group">
          ...
          <input type="text" class="form-control" id="city_name" name="city_name" placeholder="Masukkan nama kelas" value="{{ $current->name }}">
        </div>
        <div class="form-group">
          <label>Provinsi</label>
          <select class="form-control select2bs4" name="province" style="width: 100%;">
            @foreach ($province as $provinceItem)
                <option value="{{ $provinceItem->uuid }}" @selected($current->province_uuid == $provinceItem->uuid)>
                    {{ $provinceItem->name }}
                </option>
            @endForeach
          </select>
